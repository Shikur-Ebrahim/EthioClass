...
func Register(r *gin.Engine, db *sql.DB, r2 *storage.R2Client) {
	...
	r.GET("/my-learning", handlers.GetMyLearningHandler(db))
	r.POST("/lesson-progress", handlers.UpsertLessonProgressHandler(db))
	...
	r.POST("/exam/explain", handlers.ExplainExamAnswerHandler(db))
	r.POST("/quiz/explain", handlers.ExplainQuizAnswerHandler(db))
